...
def match_pxk(form_rows, pxk_totals, pxk_do_no):
    """
    Ghép số PXK với 4 pha (v3 - Cải tiến):
    Phase 0 — Invoice-consecutive greedy (chỉ khi tổng cân bằng)
    Phase 1 — Multi-PXK subset-sum (tìm tổ hợp nhiều PXK)
    Phase 2 — Iterative subset-sum (unique solution only)
    Phase 3 — Smart ambiguous fallback với proximity scoring

    Cải tiến v3:
    - Proximity scoring: Ưu tiên PXK đã dùng cho các dòng gần đó
    - Row sequence awareness: Theo dõi PXK gần nhất cho mỗi invoice
    - Smart selection: Khi có nhiều lựa chọn, chọn PXK gần nhất với PXK đã dùng
    """
    ...
    split_info = [[] for _ in range(n)]

    # v3: Track last used PXK per invoice for proximity scoring
    inv_last_pxk = defaultdict(list)  # invoice -> list of recently used PXKs

    # Build lookup tables
    mh_to_idxs     = defaultdict(list)
    inv_mh_to_idxs = defaultdict(list)
    inv_to_total_sl = defaultdict(float)
    # v3: Track row positions for sequence awareness
    inv_row_positions = defaultdict(list)  # invoice -> list of (row_num, idx)
    ...
    for fr in form_rows:
        ...
        if fr.get("inv"):
            ...
            inv_row_positions[fr["inv"]].append((fr["row"], fr["idx"]))

    # Sort row positions for each invoice
    for inv in inv_row_positions:
        inv_row_positions[inv].sort()
    ...
    # v3: Helper function to score PXK proximity
    def score_pxk_proximity(pxk, inv):
        """Score PXK based on proximity to recently used PXKs in same invoice."""
        if not inv_last_pxk[inv]:
            return 0
        try:
            pxk_num = int(pxk)
            # Calculate minimum distance to any recently used PXK
            min_dist = min(abs(pxk_num - int(p)) for p in inv_last_pxk[inv] if str(p).isdigit())
    ...
    def get_free(mh, pxk_dos, inv=None):
        ...
        if pxk_dos:
            ...
            # v3: If no matches with strict filter, try without for remaining rows
            if not filtered and inv:
                # Check if this invoice has any unassigned rows
                inv_unassigned = [i for i in all_free if form_rows[i].get("inv") == inv]
                if inv_unassigned:
                    return inv_unassigned
            ...
            if ok and unique:
                for idxs in plan.values():
                    for i in idxs:
                        ...
                        result[i] = pxk
                        status[i] = "auto"
                        # v3: Track this PXK for invoice
                        inv = form_rows[i].get("inv")
                        if inv:
    ...
    # ── Phase 3: Smart ambiguous fallback với proximity scoring ───────────────
    # v3: Group candidates by row and use proximity scoring
    row_to_candidates = defaultdict(list)  # row_idx -> [(pxk, plan)]

    for pxk in sorted(unresolved, key=pxk_sort_key):
        pxk_dos = pxk_do_no.get(pxk, set())
        plan, ok = {}, True
        for mh, target in pxk_totals[pxk].items():
            # v3: Try with invoice context for better filtering
            inv = None
            if pxk_dos:
                # Get invoice from first available D/O
                inv = list(pxk_dos)[0]
            free = get_free(mh, pxk_dos, inv)
            ...
            plan[mh] = [free[j] for j in sols[0]]
        if ok:
            # Track which rows this PXK can satisfy
            all_rows = set()
            for idxs in plan.values():
                all_rows.update(idxs)
            for i in all_rows:
                if not assigned[i]:
                    row_to_candidates[i].append((pxk, plan))

    # v3: For rows with multiple candidates, use proximity scoring
    for i, candidates in row_to_candidates.items():
        if not candidates:
            continue
        inv = form_rows[i].get("inv")

        # Add all candidates to notes
        for pxk, _ in candidates:
            if pxk not in note_pxks[i]:
                note_pxks[i].append(pxk)

        # v3: Select best candidate based on proximity scoring
        if len(candidates) == 1:
            best_pxk = candidates[0][0]
        else:
            # Score each candidate by proximity to recently used PXKs
            best_pxk = None
            best_score = -1
            for pxk, _ in candidates:
                score = score_pxk_proximity(pxk, inv) if inv else 0
                # Add small bonus for lower PXK numbers as tiebreaker
                try:
                    score += (10000 - int(pxk)) / 100000
                except:
                    pass
                if score > best_score:
                    best_score = score
        ...
        if best_pxk and not assigned[i]:
            assigned[i] = True
            result[i] = best_pxk
            status[i] = "ambiguous"
            # Track for proximity
            if inv:
                inv_last_pxk[inv].append(best_pxk)
                inv_last_pxk[inv] = inv_last_pxk[inv][-5:]
