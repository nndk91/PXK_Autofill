    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        for fname, fbytes in file_bytes_list:
            ...
            for idx, item in enumerate(res.get("items", [])):
                item_copy = dict(item)
                item_copy['line_no'] = idx + 1
                pxk_items[pxk].append(item_copy)
                pxk_totals[pxk][item["ma_hang"]] += item["so_luong"]
    return dict(pxk_totals), dict(pxk_items), pxk_dates, pxk_do_no, errors


def extract_folder_pxk_data(folder_path):
    pxk_folder = os.path.join(folder_path, "pxk")
    if not os.path.isdir(pxk_folder):
        pxk_folder = os.path.join(folder_path, "PXK")
    pxk_totals = defaultdict(lambda: defaultdict(float))
    pxk_items = defaultdict(list)
    pxk_dates = {}
...
def detect_labeled_folders(base_dir):
    """Detect folders with training data - supports multiple naming patterns."""
    folders = []
    for name in sorted(os.listdir(base_dir)):
        ...
        for fname in os.listdir(folder_path):
            if not fname.lower().endswith(".xlsx"):
                continue
            low = fname.lower()
            # Multiple patterns for empty form
            if any(k in low for k in ["không có", "khong co", "file trong", "trong", "empty", "blank"]):
                has_empty = True
            # Multiple patterns for filled form
            if any(k in low for k in ["có dữ liệu", "co du lieu", "da dien", "đã điền", "filled", "completed"]):
                has_filled = True
...
def find_forms_in_folder(folder_path):
    """Find empty and filled forms - with flexible naming."""
    empty_form = None
    filled_form = None
    for fname in os.listdir(folder_path):
        if not fname.lower().endswith(".xlsx"):
        ...
        if any(k in low for k in ["không có", "khong co", "file trong", "trong", "empty", "blank"]):
            empty_form = path
        # Filled form patterns
        elif any(k in low for k in ["có dữ liệu", "co du lieu", "da dien", "đã điền", "filled", "completed"]):
            filled_form = path
...
class ReferenceScorer:
    """Enhanced scorer with more features for better matching."""
    ...
    def __init__(self):
        ...
        # New: Direct PXK preferences
        self.inv_to_pxk_counter = defaultdict(Counter)  # inv -> {pxk: count}
        self.item_to_pxk_counter = defaultdict(Counter)  # item -> {pxk: count}
        self.inv_item_to_pxk_counter = defaultdict(Counter)  # (inv, item) -> {pxk: count}
        
        # New: Quantity patterns
        self.qty_pattern_counter = defaultdict(Counter)  # (item, qty_range) -> {pxk: count}
        
        # New: Success statistics
        self.pxk_success_rate = Counter()  # pxk -> success_count
        self.total_assignments = Counter()  # pxk -> total_count
        
        self.example_count = 0
        self.folder_count = 0
        
    def get_qty_range(self, qty):
        """Bucket quantity into ranges for pattern matching."""
        if qty < 50:
            return "0-50"
        elif qty < 100:
            return "50-100"
    ...
    def add_row_example(
        ...
        # New: Direct PXK preferences
        if inv:
            self.inv_to_pxk_counter[inv][pxk] += 1
        self.item_to_pxk_counter[item_code][pxk] += 1
        if inv:
            self.inv_item_to_pxk_counter[(inv, item_code)][pxk] += 1
            
        # New: Quantity patterns
        self.qty_pattern_counter[(item_code, qty_range)][pxk] += 1
        
        # New: Track success
        self.pxk_success_rate[pxk] += 1
    ...
    def score_candidates(
        ...
        candidate_count = len(candidate_pxks)
        qty_range = self.get_qty_range(quantity)
        prev_rank = None
        
        if prev_pxk and prev_pxk in candidate_pxks:
            prev_rank = candidate_pxks.index(prev_pxk)
        
        scored = []
        for rank, pxk in enumerate(candidate_pxks):
            score = self._score_single(
                inv, item_code, quantity, group_index, group_size,
                candidate_count, rank, prev_rank, pxk, qty_range
            )
            scored.append((pxk, score))
    ...
    def _score_single(
        self, inv, item_code, quantity, group_index, group_size,
        candidate_count, rank, prev_rank, pxk, qty_range
    ):
        """Score a single candidate with all features."""
        qty_key = round(float(quantity), 2)
        pos = position_bucket(group_index, group_size)
        score = 0.0
        
        # Original rank-based features (weighted)
        weighted_keys = [
            (12.0, ("inv_item_qty_pos", inv, item_code, qty_key, pos, candidate_count)),
            (8.0, ("item_qty_pos", item_code, qty_key, pos, candidate_count)),
            (5.0, ("item_qty", item_code, qty_key, candidate_count)),
            (3.0, ("item_pos", item_code, pos, candidate_count)),
            (1.5, ("item_only", item_code, candidate_count)),
        ]
        ...
        if prev_rank is not None:
            ...
            if counter:
                total = sum(counter.values())
                if total > 0:
                    score += 6.0 * (counter.get(rank, 0) / total)
        
        # New: Direct PXK preference features
        # Invoice -> PXK
        if inv:
            inv_counter = self.inv_to_pxk_counter.get(inv)
            if inv_counter:
                total = sum(inv_counter.values())
                if total > 0:
                    score += 15.0 * (inv_counter.get(pxk, 0) / total)
        
        # Item -> PXK
        item_counter = self.item_to_pxk_counter.get(item_code)
        if item_counter:
            total = sum(item_counter.values())
            if total > 0:
                score += 10.0 * (item_counter.get(pxk, 0) / total)
        
        # (Invoice, Item) -> PXK
        if inv:
            inv_item_counter = self.inv_item_to_pxk_counter.get((inv, item_code))
            if inv_item_counter:
                total = sum(inv_item_counter.values())
                if total > 0:
                    score += 20.0 * (inv_item_counter.get(pxk, 0) / total)
        
        # Quantity pattern -> PXK
        qty_counter = self.qty_pattern_counter.get((item_code, qty_range))
        if qty_counter:
            total = sum(qty_counter.values())
            if total > 0:
                score += 8.0 * (qty_counter.get(pxk, 0) / total)
        
        return score
    
    def get_best_candidate(self, inv, item_code, quantity, candidate_pxks, prev_pxk=None):
        """Get the best candidate based on learned preferences."""
        if not candidate_pxks:
            return None
        
        if len(candidate_pxks) == 1:
            return candidate_pxks[0]
        
        scored = self.score_candidates(
            inv, item_code, quantity, 0, 1, candidate_pxks, prev_pxk
        )
...
def load_reference_scorer(base_dir):
    """Load scorer with all available training data."""
    scorer = ReferenceScorer()
    folders = detect_labeled_folders(base_dir)
    ...
    for folder_path in folders:
        ...
        result = extract_folder_pxk_data(folder_path)
        pxk_totals = result[0]
        pxk_dates = result[2]
        pxk_do_no = result[3]
...
def match_pxk_v4(form_rows, pxk_totals, pxk_do_no, scorer=None):
    """Enhanced matching with aggressive use of learned patterns."""
    ...
    # Phase 1: Multi-PXK subset sum with scoring
    ...
    for inv in sorted(inv_to_pxks.keys(), key=lambda x: int(x) if str(x).isdigit() else 0):
        ...
        for mh, pxks_with_mh in mh_to_pxks.items():
            ...
            # Use scorer to select best combination if ambiguous
            if scorer and len(pxk_quantities) > 1:
                # Score each PXK for the rows
                prev_pxk = inv_last_pxk[inv][-1] if inv_last_pxk[inv] else None
                scored = []
                for pxk, qty in pxk_quantities:
                    score = 0
                    for idx in free_rows:
                        fr = form_rows[idx]
                        cand_score = scorer._score_single(
                            inv, mh, fr["sl"], 0, 1,
                            len(pxk_quantities), 0, None, pxk,
                            scorer.get_qty_range(fr["sl"])
                        )
                        score += cand_score
                    scored.append((pxk, score, qty))
                
                # Sort by score
                scored.sort(key=lambda x: -x[1])
                pxk_quantities = [(pxk, qty) for pxk, score, qty in scored]
    ...
    # Phase 2: Iterative subset-sum with unique solutions
    for _ in range(50):
        progress = 0
        for pxk in sorted(unresolved, key=pxk_sort_key):
            ...
            for mh, target in pxk_totals[pxk].items():
                ...
                if len(sols) > 1:
                    unique = False
                    # Use scorer to pick best solution
                    if scorer:
                        best_sol = None
                        best_score = -1
                        for sol in sols:
                            ...
                            for j in sol:
                                ...
                                inv = fr.get("inv")
                                candidate_pxks = [
                                    p for p in sorted(inv_to_pxks.get(inv, []), key=pxk_sort_key)
                                    if pxk_totals.get(p, {}).get(mh, 0) > 0
                                ]
                                if pxk in candidate_pxks:
                                    rank = candidate_pxks.index(pxk)
                                    score += scorer.score_rank(
                                        inv, mh, fr["sl"], 0, 1, len(candidate_pxks), rank
                                    )
                            if score > best_score:
                                best_score = score
                                best_sol = sol
    ...
    # Phase 3: Beam search with enhanced scoring
    ...
    for (inv, mh), rows in sorted(
        ...
        # Use scorer for better initial ranking
        if scorer and len(candidate_pxks) > 1:
            prev_pxk = inv_last_pxk[inv][-1] if inv and inv_last_pxk[inv] else None
            scored = scorer.score_candidates(
                inv, mh, rows[0]["sl"] if rows else 0,
                0, len(rows), candidate_pxks, prev_pxk
            )
            # Reorder candidates by score
            scored_pxks = [pxk for pxk, score in scored]
            # Keep original order for same scores but prioritize high scores
            candidate_pxks = scored_pxks
        ...
        for pos, fr in enumerate(rows):
            ...
            for state in beam:
                for rank, pxk in enumerate(candidate_pxks):
                    ...
                    # Enhanced scoring with scorer
                    learned = 0.0
                    if scorer:
                        learned = scorer._score_single(
                            inv, mh, fr["sl"], pos, len(rows),
                            len(candidate_pxks), rank, state["prev_rank"],
                            pxk, scorer.get_qty_range(fr["sl"])
                        )
