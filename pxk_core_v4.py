...
# ── FIX 1: Sheet name lookup strips trailing spaces + fallback is robust ──────
def get_data_sheet(wb):
    """Return the main data worksheet.

    Looks for known sheet names (trimmed), then falls back to the active sheet.
    Previously the literal string "XUẤT " (with trailing space) was used, which
    silently missed sheets named "XUẤT" (no space).
    """
    for name in wb.sheetnames:
        if name.strip() in ("XUẤT", "Sheet1"):
            return wb[name]
    return wb.active
...
# ── FIX 2: Guard against ws.max_row being None ───────────────────────────────
...
# ── FIX 3: detect_labeled_folders — "không có dữ liệu" falsely matched "có dữ liệu"
#    because "có dữ liệu" is a substring of "không có dữ liệu".
#    Fix: check has_empty FIRST with elif so has_filled is never set for empty-form files.
def detect_labeled_folders(base_dir):
    """Find training folders that contain both an empty form and a filled form.

    A folder qualifies when its top-level directory contains a PXK/ subfolder
    AND at least two .xlsx files whose names identify them as the unfilled form
    and the filled (reference) form respectively.

    BUG FIXED: previously used two separate ``if`` statements, so a file named
    "FORM KHÔNG CÓ DỮ LIỆU.xlsx" set *both* has_empty and has_filled because
    "có dữ liệu" is a substring of "không có dữ liệu".  Changed to if/elif.
    """
    folders = []
    for name in sorted(os.listdir(base_dir)):
        ...
        for fname in os.listdir(folder_path):
            if not fname.lower().endswith(".xlsx"):
                continue
            low = fname.lower()
            # FIX: use elif so the empty-form file never triggers has_filled
...
# ── FIX 4: lru_cache key is the *resolved* absolute path so that "." and the
#    absolute equivalent always hit the same cache slot, and Streamlit hot-reloads
#    don't serve a scorer built from a different working directory.
@lru_cache(maxsize=4)
def load_reference_scorer(base_dir):
    """Load and cache a ReferenceScorer from training folders under *base_dir*.

    FIX: resolve *base_dir* to an absolute path before caching so that callers
    using relative paths (e.g. ``"."``) always hit the same cache entry even if
    the process working directory changes between Streamlit re-runs.
    """
...
def match_pxk_v4(form_rows, pxk_totals, pxk_do_no, scorer=None):
    ...
    for (inv, mh), rows in sorted(
        ...
        # ── FIX 5: skip beam search entirely when rows is empty ──────────────
        if not rows:
            continue
        ...
        for pos, fr in enumerate(rows):
            ...
            # Loop completed without break → all rows were processed
            best = max(
                beam,
                key=lambda state: (
                    len(state["assignments"]),
            ...
            # ── FIX 6: guard against best having fewer assignments than rows ──
            if len(best["assignments"]) < len(rows):
                continue
            for pos, fr in enumerate(rows):
...
# ── FIX 7: Use NamedTemporaryFile instead of deprecated tempfile.mktemp ──────
def build_output_excel(wb_bytes, form_rows, result, status_list, note_pxks, pxk_dates):
    ...
    # FIX: use NamedTemporaryFile with delete=False instead of mktemp
    with tempfile.NamedTemporaryFile(suffix=".xlsx", delete=False) as tmp_fh:
        tmp = tmp_fh.name
        tmp_fh.write(wb_bytes)
